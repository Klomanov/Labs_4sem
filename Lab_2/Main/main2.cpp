...
public:
    ...
    void doTimeStep(double cur_time, double tau)
    {
        ...
        for (unsigned int i = 0; i < nodes.size(); i++)
        {
            ...
            double x0 = nodes[i].x0, y0 = nodes[i].y0, z0 = nodes[i].z0;

            nodes[i].x = -sin(2 * M_PI / (50 * tau) * cur_time) * (c_x - x0) * 0.75 * exp(-cur_time * 1.5) + x0;
            nodes[i].y = -sin(2 * M_PI / (50 * tau) * cur_time) * (c_y - y0) * 0.75 * exp(-cur_time * 1.5) + y0;
            nodes[i].z = -sin(2 * M_PI / (50 * tau) * cur_time) * (c_z - z0) * 0.75 * exp(-cur_time * 1.5) + z0;
    ...
    void snapshot(unsigned int snap_number)
    {
        ...
        string fileName = "./Snapshots/Moving/AsianGirl-step-" + std::to_string(snap_number) + ".vtu";
