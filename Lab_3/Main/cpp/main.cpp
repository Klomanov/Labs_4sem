...
class Source : public Expression
{
  void eval(Array<double> &values, const Array<double> &x) const
  {
    ...
    double dy = x[1] - 0.5;
    double dz = x[2] - 0.5;
    values[0] = 100 * exp(-(dx * dx + dy * dy + dz * dz));
  }
};
...
class dUdN : public Expression
{
  void eval(Array<double> &values, const Array<double> &x) const
  {
    values[0] = sin(5 * x[0] + 5 * x[1]);
  }
};

class dirFun : public Expression
{
  void eval(Array<double> &values, const Array<double> &x) const
  {
    values[0] = 20 * sin(5 * x[0] + 5 * x[1]) + 10 * cos(5 * x[1] + 5 * x[2]);
  }
};
...
int main()
{
  ...
  auto u0 = std::make_shared<dirFun>();
